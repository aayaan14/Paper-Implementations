...
class HSSkipGramDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        L = self.max_path_len
        pad_len = L - len(path_nodes)

        # pad nodes and codes
        path_nodes_padded = path_nodes + [0] * pad_len
        path_codes_padded = path_codes + [0] * pad_len

        # mask: 1 for real positions, 0 for padded ones
        mask = [1] * len(path_nodes) + [0] * pad_len

        return (
            torch.tensor(center_id, dtype=torch.long),
            torch.tensor(path_nodes_padded, dtype=torch.long),
            torch.tensor(path_codes_padded, dtype=torch.float),   # float for BCE target
            torch.tensor(mask, dtype=torch.float)                 # float mask
        )
...
class HS_SkipGramModel(nn.Module):
    ...
    def __init__(self, vocab_size, n_embd):
        ...
        # Initialize
        nn.init.xavier_uniform_(self.in_embd.weight)
        nn.init.xavier_uniform_(self.out_embd.weight)

    def forward(self, center_words, path_nodes, path_codes, mask):
        """
        Args:
        ...
        logits = torch.sum(node_emb * center_emb.unsqueeze(1), dim=2)  # [B, L]
        
        bce = F.binary_cross_entropy_with_logits(logits, path_codes, reduction='none')  # [B, L]
        masked_bce = bce * mask
        
        denom = mask.sum(dim=1)  
        denom = torch.clamp(denom, min=1.0)
        
        loss_per_example = masked_bce.sum(dim=1) / denom  
...
for step in range(max_steps):
    ...
    # Get batch
    try:
        center, path_nodes, path_codes, path_mask = next(data_iter)
    ...
    # Forward
    loss = model(center, path_nodes, path_codes, path_mask)
